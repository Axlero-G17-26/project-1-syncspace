...
export default function Whiteboard({
  ...
  const [playbackSpeed, setPlaybackSpeed] = useState(1); // Speed multiplier: 1x, 2x, 5x, 10x
  ...
  useEffect(() => {
    ...
    // Slice strokes if in replay mode
    ...
    // Draw active drawing stroke if drawing (only when not in replay mode)
  ...
  }, [strokes, canvasSize, isDrawing, brushColor, brushWidth, currentTool, isReplayMode, replayIndex]);

  // 3. Playback animation interval loop
  useEffect(() => {
    if (!isPlaying || !isReplayMode) return;

    // Speed multiplier: 1x -> 600ms, 2x -> 300ms, 5x -> 120ms, 10x -> 60ms
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 overflow-hidden select-none" id="whiteboard-container">
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          <button
            ...
            onClick={() => {
              ...
              if (nextReplay) {
                setReplayIndex(strokes.length);
              } else {
                setIsPlaying(false);
              }
            }}
            className={`p-2 rounded-lg transition-all cursor-pointer flex items-center justify-center gap-1.5 text-xs font-semibold ${
              isReplayMode
          ...
          >
            ...
            <span className="hidden sm:inline">History Replay</span>
          </button>
      ...
      >
        ...
        {/* Replay Controller Glassmorphic Panel */}
        {isReplayMode && (
